...
function directive(querrySelectorAll, attributesList) {
    ...
    for (var i = 0; i < NodeList.length; i++) {
        ...
        for (var i_1 = 0; i_1 < attributesList.length; i_1++) {
            eachAttribute = attributesList[i_1];
            if (NodeList[i_1].getAttribute(eachAttribute) === undefined) {
                Obj[eachAttribute] = undefined;
                if (NodeList[i_1].id !== undefined)
                    for (var each in document.querySelectorAll("[" + eachAttribute + "]"))
                        if (each["id"] !== undefined)
                            if (each["id"] === NodeList[i_1].id)
                                Obj[eachAttribute] = true;
            }
            else
                Obj[eachAttribute] = NodeList[i_1].getAttribute(eachAttribute);
        }
...
[Symbol.iterator] = function() {
    let mylist = (this.container) ? this.container.items : ((this.pages) ? this.pages : []);
    return { // this is the iterator object, returning a single element, the string "bye"
    next: function() {
          if (this._index < this._list.length ) {
            this._index += 1;
            return { value: this._list[this._index - 1], done: false };
          } else {
            return { value: undefined, done: true };
          }
        },
