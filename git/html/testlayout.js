...
function mystartup() {
    ...
    for (i = 0; i < menuKeys.length; i++) {
        ...
        out = "";
        for (j = 0; j <= i; j++) {
            out += "<div onclick=\'myClassSelect(\"" + menuKeys[j] + "\")\'>";
            out += menuKeys[j] + "<br>";
            out += "</div>";
        }
        for (j = 0; j < oldlist.length; j++) {
            extra = "onclick='Item.setPage(\"Pages\", \"" + menuKeys[i] + "_" + oldlist[j] + "\")'";
            mystyle = " style=\"cursor: pointer\;\"";
            out += '<div class="nav' + oldlist[j] + '" ' + extra + mystyle + '>';
...
//  attach event management
    ...
    eleById("selectSubClass").onchange = function () {
        ...
        var value = e.options[e.selectedIndex].text;
        Item.setPage("Pages", currentMenu() + "_" + value);
    };
    setTimeout(function () {
        myClassSelect("Item");
    }, 2);
...
// ts is object key=id value=html as text
    ...
    for (i = 0; i < tsKeys.length; i++) {
        ...
        tempEl = htmlToElement(ts[tsKeys[i]]);
...
//        NavItemBlock,
    ...
    var RightSideOfXlargeLayoutItemBlock = h("RightSideOfXlargeLayoutItemBlock", "800px", "500px", "900px",
        v("RightSideOfXlargeLayout", "400px", "200px", "600px",
//                                                    NavItemBlock,
            LayoutButtonsItemBlock,
            I("jsonTree", "100%")
        ),
