...
function clearClasses() {
    for (var each of ["sa", "xs", "ss", "sm", "sl", "sx"]) {
        document.getElementById(each).removeAttribute("class");
    }
}
...
function myClassSelect(classname) {
    ...
    Item.setPage("Pages",classname+"_"+subname);
}
function pg(page) {
    var menu = "Functions";
    if (page.indexOf("_") !== -1) {
...
function updateDisplayOld() {
    if (Item.get("Pages").currentPage !== currentPage) {
//      alert("Page Change");
        currentPage = Item.get("Pages").currentPage;
    }
